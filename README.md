...
| `tools/` | `scabbards.txt` (covered scabbard definitions), `gen_variants.py`, `pack_bundle.sh` | - |
...
definition listed in `tools/scabbards.txt`. While the invisible marker item of a
...
Scabbards not listed in `tools/scabbards.txt` keep their own look. Definition
names that are not loaded (a mod that is not installed) are harmless.

## Adding a scabbard

1. Add a line `name | category | origin` to `tools/scabbards.txt`
   (`name` is the item definition name of the scabbard).
